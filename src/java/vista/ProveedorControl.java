...
public class ProveedorControl extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (acc.equals("Grabar")) {
            ...
            response.sendRedirect("ProveedorMensaje.jsp");
        }

        if (acc.equals("Buscar")) {

            String cod = request.getParameter("cod");
            ProveedorServicioImp proServicioImp = new ProveedorServicioImp();
            Object[] f = proServicioImp.buscar(cod);
            if (f != null) {
                request.getSession().setAttribute("f", f);
                response.sendRedirect("ProveedorEditar.jsp");
            } else {
                request.getSession().setAttribute("msg", "No existe el proveedor solicitado");
                response.sendRedirect("ProveedorMensaje.jsp");
            }
        }

        if (acc.equals("Listar")) {
            List lis = new ProveedorServicioImp().listar();
